...
async def test_service_type_filter_narrows_bookings(client: AsyncClient, engine) -> None:
    ...
    resp = await client.patch(
        ...
        json={
            "services": [
                ...
                {"service_type": "resume_writer", "duration_minutes": 30, "price_usd": 50.0},
            ]
        },
    ...
    _, cust_headers = await _seeker(client)
    for service_type, hour in (("immigration_specialist", 9), ("resume_writer", 11)):
        resp = await client.post(
            BOOKINGS,
            json={
                "advisor_id": advisor_id,
    ...
    data = resp.json()["data"]
    assert len(data) == 1
    assert data[0]["service_type"] == "resume_writer"
